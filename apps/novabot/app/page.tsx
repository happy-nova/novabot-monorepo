...
export default function NovaHome() {
  ...
  const startAudioAnalysis = useCallback((audio: HTMLAudioElement) => {
    try {
      ...
      const sampleLevel = () => {
        ...
        // Generate smooth sine wave modulated by audio level
        if (waveformRef.current) {
          ...
          if (path) {
            ...
            const amplitude = level * 40; // Wave height based on audio
            const frequency = 4 + level * 2; // 4-6 waves, slightly more when louder
            const phase = Date.now() / 200; // Animate wave movement
            ...
            for (let i = 0; i <= points; i++) {
              ...
              if (i === 0) {
                ...
                // Use quadratic curves for smooth sine
                const prevX = ((i - 1) / points) * width;
                const prevT = ((i - 1) / points) * Math.PI * 2 * frequency + phase;
                const prevY = midY - Math.sin(prevT) * amplitude;
                const cpX = (prevX + x) / 2;
                const cpY = midY - Math.sin((prevT + t) / 2) * amplitude * 1.15; // Slight overshoot for rounder peaks
  ...
  const navigateEntity = useCallback((direction: 'prev' | 'next') => {
    ...
    const total = constellationEntities.length;
    const newIndex = direction === 'next' 
      ? (activeEntity + 1) % total 
      : (activeEntity - 1 + total) % total;
              ...
              {selectedEntity && (
                <div className="constellation-modal" onClick={() => setActiveEntity(null)}>
                  ...
                  <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); navigateEntity('prev'); }}
                    onMouseEnter={handleHoverEnter}
                    onMouseLeave={handleHoverLeave}
                  ...
                  <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); navigateEntity('next'); }}
                    onMouseEnter={handleHoverEnter}
                    onMouseLeave={handleHoverLeave}
                      ...
                      {constellationEntities.map((entity, i) => 
                        constellationEntities.slice(i + 1).map((other, j) => (
                          <line 
                            ...
                            x1={entity.position.x} y1={entity.position.y}
